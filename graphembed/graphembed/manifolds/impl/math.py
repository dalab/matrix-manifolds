"""
Poincare manifold and stereographical projection of sphere utility functions.

Functions for math on Poincare ball model are mostly taken from a well written
paper by Octavian-Eugen Ganea (2018) [1]_.
...
class Arsinh(torch.autograd.Function):
    ...
    def forward(ctx, x):
        ctx.save_for_backward(x)
        z = x.double()
        return (z + torch.sqrt_(1 + z.pow(2))).clamp_min_(MIN_NORM).log_().to(
                x.dtype)
...
"""
The following function selects the appropriate trigonometric function (normal or
hyperbolic) depending on the value of c. They include some additional checks to
make the common case go fast (all c pos / all c neg).
"""
...
def tan_func(x, c):
    ...
    if num_greater_zero == c_size:
        ...
        result = torch.where(c_greater_zero, tanh_reults, tan_results)
        return result
...
def project(x, *, c=1.0, dim=-1, eps=None):
    ...
    ----------
    x : tensor
        point on the Universal manifold
    c : float|tensor
        negative curvature
    dim : int
...
def lambda_x(x, *, c=1.0, keepdim=False, dim=-1):
    r"""
    Compute the conformal factor :math:`\lambda^c_x` for a point on the
    manifold.
...
def _lambda_x(x, c, keepdim: bool = False, dim: int = -1):
    #assert((1 - c * x.pow(2).sum(dim=dim, keepdim=keepdim)).min() > 0)
    return 2 / (
            1 - c * x.pow(2).sum(dim=dim, keepdim=keepdim)).clamp_min(MIN_NORM)


def inner(x, u, v, *, c=1.0, keepdim=False, dim=-1):
    r"""
    Compute inner product for two vectors on the tangent space w.r.t Riemannian
    metric on manifold.
...
def _inner(x, u, v, c, keepdim: bool = False, dim: int = -1):
    return _lambda_x(
            x, c, keepdim=True, dim=dim)**2 * (u * v).sum(
                    dim=dim, keepdim=keepdim)


def norm(x, u, *, c=1.0, keepdim=False, dim=-1):
    r"""
    Compute vector norm on the tangent space w.r.t Riemannian metric on the
    manifold.
...
def _norm(x, u, c, keepdim: bool = False, dim: int = -1):
    return _lambda_x(
            x, c, keepdim=keepdim, dim=dim) * u.norm(
...
def mobius_coadd(x, y, *, c=1.0, dim=-1):
    ...
    Addition operation :math:`\oplus_c` is neither associative, nor commutative.
    Coaddition, or cooperation in Gyrogroup is an associative operation that is
    defined as follows.
...
def mobius_cosub(x, y, *, c=1.0, dim=-1):
    r"""
    Compute Mobius cosubtraction operation.

    Mobius cosubtraction is defined as follows:
...
def mobius_scalar_mul(r, x, *, c=1.0, dim=-1):
    r"""
    Compute left scalar multiplication on manifold.
...
def _mobius_scalar_mul(r, x, c, dim: int = -1):
    ...
    res_c = tan_func(r * arctan_func(sqrt_abs_c * x_norm, c),
                     c) * x / (x_norm * sqrt_abs_c)
    return res_c
...
def dist(x, y, *, c=1.0, keepdim=False, dim=-1):
    ...
    """
    # TODO: this is the code for distance emulation (doesn't help)
    #R = 1.0/torch.sqrt(torch.abs(c))
    #c_emul = torch.sign(c)
    #return R * _dist(x/R, y/R, c_emul, keepdim=keepdim, dim=dim)
...
def _dist(x, y, c, keepdim: bool = False, dim: int = -1):
    sqrt_abs_c = torch.abs(c)**0.5
    dist_c = arctan_func(
            sqrt_abs_c * _mobius_add(-x, y, c, dim=dim).norm(
                    dim=dim, p=2, keepdim=keepdim), c)
    return dist_c * 2 / sqrt_abs_c


def dist0(x, *, c=1.0, keepdim=False, dim=-1):
    r"""
    Compute geodesic distance on the manifold to origin.
...
def geodesic(t, x, y, *, c=1.0, dim=-1):
    ...
    points but in a Riemannian manifold. In universal manifold model, the path
    ...
    Moreover, as geodesic path is not only the shortest path connecting points
    on the manifold. This definition also requires local distance minimization
    and thus another property appears:
...
# TODO: from here on I haven't adapted the docs to the universal manifold yet.


def expmap(x, u, *, c=1.0, dim=-1):
    r"""
    Compute exponential map on the manifold.

    Exponential map for manifold. This is tightly related with :func:`geodesic`.
    Intuitively exponential map is a smooth constant travelling from starting
    point :math:`x` with speed :math:`u`.

    A bit more formally this is travelling along curve :math:`\gamma_{x, u}(t)`
    such that
...
def _expmap(x, u, c, dim: int = -1):
    ...
    second_term = (tan_func(
            sqrt_abs_c / 2 * _lambda_x(x, c, keepdim=True, dim=dim) * u_norm, c)
                   * u / (sqrt_abs_c * u_norm))
...
def _logmap(x, y, c, dim: int = -1):
    ...
    sqrt_abs_c = torch.abs(c)**0.5
    return 2 / sqrt_abs_c / lam * arctan_func(sqrt_abs_c * sub_norm,
                                              c) * sub / sub_norm
...
def _mobius_matvec(m, x, c, dim: int = -1):
    ...
    res_c = tan_func(mx_norm / x_norm * arctan_func(sqrt_abs_c * x_norm, c),
                     c) * mx / (mx_norm * sqrt_abs_c)
...
def _mobius_pointwise_mul(w, x, c, dim: int = -1):
    ...
    res_c = tan_func(wx_norm / x_norm * arctan_func(sqrt_abs_c * x_norm, c),
                     c) * wx / (wx_norm * sqrt_abs_c)
...
def _parallel_transport(x, y, u, c, dim: int = -1):
    return (_gyration(y, -x, u, c, dim=dim) * _lambda_x(
            x, c, keepdim=True, dim=dim) / _lambda_x(
                    y, c, keepdim=True, dim=dim))
...
def _parallel_transport0(y, v, c, dim: int = -1):
    #assert((v * (1 - c * y.pow(2).sum(dim=dim, keepdim=True))).min()>0)
...
def _parallel_transport0back(x, v, c, dim: int = -1):
    #assert((v / (1 - c * x.pow(2).sum(dim=dim, keepdim=True))).min()>0)
